from lib.algos.sigcwgan import SigCWGANConfig, MSigCWGANConfig
...
SIGCWGAN_CONFIGS = dict(
    ...
    VAR1=SigCWGANConfig(
        ...
        sig_config_future=SignatureConfig(depth=3, augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag()])),
    ),
    VAR2=SigCWGANConfig(
        mc_size=1000,
...
# if time series is martingale difference (e.g. log-returns) add Cumsum() transform to map to martingale
MSIGCWGAN_CONFIGS = dict(
    STOCKS_SPX=MSigCWGANConfig(
        mc_size=1000,
        sig_config_past=SignatureConfig(depth=3, augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag()])),
        sig_config_future=SignatureConfig(depth=3, augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag()])),
        martingale_indices = [True, False]
    ),
    STOCKS_SPX_DJI=MSigCWGANConfig(
        mc_size=1000,
        sig_config_past=SignatureConfig(depth=2, augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag()])),
        sig_config_future=SignatureConfig(depth=2, augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag()])),
        martingale_indices = [True, False, True, False]
    ),
)